    ...
    private function calendarMonth($month)
    {
        ...
        for ($i = 0; $i < 31; $i = $i + 3) {
            if (isset($timetables[$i]) and isset($timetables[$i + 1]) and isset($timetables[$i + 2])) {
                $inline_keyboard->row(
                    Keyboard::inlineButton(["text" => Carbon::parse($timetables[$i])->format('d'), 'callback_data' => '/_' . Carbon::parse($timetables[$i])->format('Y-m-d')]),
                    Keyboard::inlineButton(["text" => Carbon::parse($timetables[$i + 1])->format('d'), 'callback_data' => '/_' . Carbon::parse($timetables[$i + 1])->format('Y-m-d')]),
                    Keyboard::inlineButton(["text" => Carbon::parse($timetables[$i + 2])->format('d'), 'callback_data' => '/_' . Carbon::parse($timetables[$i + 2])->format('Y-m-d')])
                );
